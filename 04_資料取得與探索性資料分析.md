...
```python
# 安裝 ucimlrepo 套件（在 Colab 環境可直接執行）
# !pip install ucimlrepo
from ucimlrepo import fetch_ucimlrepo

# 載入 Bank Marketing 資料集 (UCI id=222)
...
```python
# 1. 檢視教育程度 (education) 包含哪些不重複類別名稱
print("教育程度 (unique):", df['education'].unique())
# 2. 計算職業 (job) 特徵值種類
print("職業類別數量 (nunique):", df['job'].nunique())
# 3. 計算婚姻狀況 (marital) 出現次數與比例 (%)
print("\n--- 婚姻狀況個數統計 ---")
print(df['marital'].value_counts())
print("\n--- 婚姻狀況百分比例 (%) 統計 ---")
print(df['marital'].value_counts(normalize=True) * 100)
# 4. 【核心重點】計算目標欄位 (y) 次數與比例 (%)
print("\n--- 目標欄位 (y) 訂閱次數統計 ---")
...
# 2. 使用 pd.cut() 將通話時長切割為 3 個長度區間 (秒)
...
df['duration_group'] = pd.cut(
    df['duration'], bins=duration_bins,
...
job_pct = job_unknown_count / len(df) * 100
print(f"個數: {job_unknown_count} 筆 ({job_pct:.2f}%)")

print("\n--- 教育 (education) 中 'unknown' 統計 ---")
edu_unknown_count = (df['education'] == 'unknown').sum()
edu_pct = edu_unknown_count / len(df) * 100
print(f"個數: {edu_unknown_count} 筆 ({edu_pct:.2f}%)")
```
...
# 2. 繪製年齡區間 (age_group) 分佈長條圖
sns.countplot(
    data=df, x='age_group',
    ax=axes[1], palette='Oranges'
)
axes[1].set_title(
    'Customer Count by Age Group (pd.cut Result)',
    fontsize=12, fontweight='bold'
)
...
```python
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
# 1. 繪製通話時長直方圖，紅虛線標註 90% 分位點 (557秒)
q90_duration = df['duration'].quantile(0.9)
sns.histplot(
    data=df, x='duration', bins=50,
    kde=True, ax=axes[0], color='skyblue'
)
axes[0].axvline(
    q90_duration, color='red', linestyle='--',
    linewidth=2,
    label=f'90th Percentile ({q90_duration:.0f}s)'
)
axes[0].set_title(
    'Call Duration Distribution with 90th Quantile Line',
    fontsize=12, fontweight='bold'
)
...
# 2. 繪製通話時長箱形圖檢視離群值
sns.boxplot(
    data=df, x='duration', ax=axes[1], color='salmon'
)
axes[1].set_title(
    'Call Duration Boxplot (Outliers Visualized)',
    fontsize=12, fontweight='bold'
)
...
# 觀察不同通話長度區間下，最終是否訂閱 (y) 的顧客對比
sns.countplot(
    data=df, x='duration_group', hue='y',
    palette={'no': 'lightgrey', 'yes': 'limegreen'}
)
plt.title(
    'Subscription Status (y) Across Call Duration Bins (pd.cut)',
...
plt.legend(
    title='Subscribed (y)',
    labels=['No (Unsuccessful)', 'Yes (Successful)']
)
plt.tight_layout()
plt.show()
```
