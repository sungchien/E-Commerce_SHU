...
print(f"Bank Marketing 全局總訂閱轉換率: "
...
age_conversion = (
    df.groupby('age_group')['conversion'].mean() * 100
)
print("--- 各年齡層訂閱轉換率 (%) ---")
...
# 2. 多維度交叉下鑽：年齡層 × 通話時長 樞紐表 (%)
pivot_age_duration = pd.pivot_table(
    data=df,
    values='conversion',
    index='age_group',
...
60+ (高齡/退休)  12.38%   38.88%    51.57%    59.89%
```
...
# 1. 熱力圖一：年齡層 × 聯絡方式
sns.heatmap(
    pivot_age_contact, annot=True, fmt=".1f",
    cmap="YlGnBu", ax=axes[0],
    cbar_kws={'label': 'Conversion Rate (%)'}
)
axes[0].set_title(
    'Conversion Rate (%) by Age & Contact Method',
...
# 2. 熱力圖二：年齡層 × 通話時長區間
sns.heatmap(
    pivot_age_duration, annot=True, fmt=".1f",
    cmap="YlOrRd", ax=axes[1],
    cbar_kws={'label': 'Conversion Rate (%)'}
)
axes[1].set_title(
    'Conversion Rate (%) by Age & Duration Group',
    fontsize=12, fontweight='bold'
)
